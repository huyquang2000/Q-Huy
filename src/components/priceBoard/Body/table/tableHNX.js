...
export default function TableHNX() {
    ...
    const TableHNX = HNXData.map((item, i) => {
        return (
            <>
                <tbody>
                    <tr key={i}>
                        ...
                        <td className="red-color">{CalBoard(item.PT_TOTAL_TRADED_QTTY, 1000)}</td>
                        <td className="red-color">{CalBoard(item.PT_TOTAL_TRADED_VALUE, 1000)}</td>
